...
from repositories.data.llm.LLMProviderConfigRepository import (
...
class LLMProviderConfigRepositoryPostgres(LLMProviderConfigRepository):
    ...
    async def add(
        ...
        provider_config_po = LLMProviderConfigPO.from_llm_provider_config(
            llm_provider_config
        )
...
class LLMProviderConfigPO(PostgresBasePO):
    ...
    workspace_uid: Mapped[str] = mapped_column(
        String(32), nullable=False, comment="工作空间uid"
    )
    provider_name: Mapped[str] = mapped_column(
        String(32), nullable=False, comment="provider key"
    )
    provider_credential: Mapped[str] = mapped_column(
        String(), nullable=False, comment="provider 凭证"
    )

    @classmethod
    def from_llm_provider_config(
        cls, llm_provider_config: LLMProviderConfig
    ) -> LLMProviderConfigPO:
        """
