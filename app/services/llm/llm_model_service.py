...
llm_system_model_config_cache: CacheDecorator[dict[ModelType, LLMModelConfig]] = (
    cache_decorator_builder.build(
        ...
        deserialize=lambda json_content: {
            ModelType(model_type.upper()): LLMModelConfig.model_validate(model_config)
...
async def _get_models(
    ...
    llm_providers = (
        [model_provider_factory.get_provider(provider_name)]
        if provider_name
    ...
    configured_provider_names = set(
        map(lambda p: p.provider_name, configured_providers)
    )

    # 如果指定only_active，则获取已配置的Providers
    if only_active:
        llm_providers = filter(
            lambda p: p.provider in configured_provider_names, llm_providers
        )
    if not llm_providers:
    ...
    for llm_provider in llm_providers:
        ...
        provider_with_models.append(
            ProviderWithModelsSchema(
                provider=llm_provider.provider_schema,
                models=models,
                status=(
                    ProviderStatus.ACTIVE
                    if llm_provider.provider in configured_provider_names
...
async def check_and_renew_llm_model_config(
    workspace_uid: str, model_type: ModelType, model_config: LLMModelConfig
) -> LLMModelConfig:
    """
    校验参数，并过滤有效参数
    :param workspace_uid: 工作空间UID
    ...
    """
    provider_with_models_schemas = await _get_models(
        workspace_uid, model_config.provider_name, model_type
    )
    ...
    provider_with_models_schema: ProviderWithModelsSchema = next(
        filter(
            lambda s: s.provider.provider == model_config.provider_name,
            provider_with_models_schemas,
        ),
        None,
    )
    ...
    model_schema = next(
        filter(
            lambda m: m.model == model_config.model_name,
            provider_with_models_schema.models,
        ),
        None,
    )
    if not model_schema:
...
def _check_and_renew_llm_model_config(
    model_config: LLMModelConfig, model_schema: ModelSchema
) -> LLMModelConfig:
    """
    校验参数，并过滤有效参数
    :param model_config: 模型配置
    ...
    # 校验必须参数
    for schema_parameter in [
        schema_parameter
        for schema_parameter in schema_parameters
        if schema_parameter.rules and schema_parameter.rules.required
    ]:
        if (
            schema_parameter.name not in valid_parameters
            or not valid_parameters[schema_parameter.name]
        ):
            raise LLMExistsError(
                f"LLM模型{model_config.provider_name}:{model_config.model_name}"
    ...
    for schema_parameter in [
        schema_parameter
        for schema_parameter in schema_parameters
        if schema_parameter.value_type == "digit"
        and schema_parameter.name in valid_parameters
    ]:
        schema_min = dict_get(schema_parameter.field_props, "min", None)
        if (
            schema_min is not None
            and valid_parameters[schema_parameter.name] < schema_min
        ):
            raise LLMExistsError(
                f"LLM模型{model_config.provider_name}:{model_config.model_name}"
        ...
        if (
            schema_max is not None
            and valid_parameters[schema_parameter.name] > schema_max
        ):
            raise LLMExistsError(
                f"LLM模型{model_config.provider_name}:{model_config.model_name}"
...
async def _add_system_config(
    workspace_uid: str, llm_model_config: dict[ModelType, LLMModelConfig]
) -> dict[ModelType, LLMModelConfig]:
    """
    添加 工作空间 系统默认模型配置
    :param workspace_uid: 空间UID
    ...
    # 参数校验
    model_configs = {
        model_type: await check_and_renew_llm_model_config(
            workspace_uid, model_type, model_config
        )
        for model_type, model_config in llm_model_config.items()
    }

    exists_model_config = (
        await llm_model_config_repository.find_system_models_by_workspace(workspace_uid)
    )
    return (
        await llm_model_config_repository.add_system_models(
            workspace_uid, model_configs
        )
        if exists_model_config is None
        else await llm_model_config_repository.update_system_models(
            workspace_uid, model_configs
        )
    )
