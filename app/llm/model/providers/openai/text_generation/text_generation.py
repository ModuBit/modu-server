...
class OpenAITextGenerationModel(TextGenerationModel):
    def chat_model(self,
        ...
        model_params = dict_map_values(model_parameters, lambda k, v: {"type": v} if k == "response_format" else v)
        ...
        model_params = dict_filter_none_values({
            ...
            # 其他模型参数
            "model_kwargs": dict_exclude_keys(model_params,
                                              ["streaming", "temperature", "max_tokens", "stop"]),
        })
    ...
    async def validate_credentials(self, credentials: dict, model: str | None = None) -> None:
        try:
            ...
            chat_model = self.chat_model(provider_credential=credentials,
